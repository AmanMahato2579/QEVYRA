...
export async function PATCH(
  ...
  // Restaurant-scoped profile / toggles
  const restaurantUpdate: Record<string, unknown> = {};
  if (data.name !== undefined) restaurantUpdate.name = data.name;
  if (data.phone !== undefined) restaurantUpdate.phone = data.phone;
  if (data.address !== undefined) restaurantUpdate.address = data.address;
  if (data.description !== undefined) restaurantUpdate.description = data.description;
  if (data.tableLimit !== undefined) restaurantUpdate.tableLimit = data.tableLimit;
  if (data.bookingsEnabled !== undefined) restaurantUpdate.bookingsEnabled = data.bookingsEnabled;
  if (data.brandColor !== undefined) restaurantUpdate.brandColor = data.brandColor;
  if (data.isActive !== undefined) restaurantUpdate.isActive = data.isActive;

  // Business-scoped subscription / plan / star
  const businessUpdate: Record<string, unknown> = {};
  if (data.plan !== undefined) businessUpdate.plan = data.plan;
  if (data.subscriptionStatus !== undefined) businessUpdate.subscriptionStatus = data.subscriptionStatus;
  if (data.subscriptionStart !== undefined)
    businessUpdate.subscriptionStart = data.subscriptionStart === null ? null : new Date(data.subscriptionStart);
  if (data.subscriptionExpiresAt !== undefined)
    businessUpdate.subscriptionExpiresAt = data.subscriptionExpiresAt === null ? null : new Date(data.subscriptionExpiresAt);
  if (data.neverExpires !== undefined) businessUpdate.neverExpires = data.neverExpires;
  if (data.autoOff !== undefined) businessUpdate.autoOff = data.autoOff;
  if (data.starNote !== undefined) businessUpdate.starNote = data.starNote;
  if (data.starNumber !== undefined) businessUpdate.starNumber = data.starNumber;
  ...
  if (data.starNumber !== undefined && data.starNumber !== null) {
    ...
    if (!data.neverExpires && business.neverExpires) businessUpdate.neverExpires = true;
    if (businessUpdate.subscriptionExpiresAt === undefined) businessUpdate.subscriptionExpiresAt = null;
    if (businessUpdate.subscriptionStart === undefined) businessUpdate.subscriptionStart = new Date();
  }

  // Extend by N days from now (or current future expiry), and clear neverExpires.
  if (data.extendDays !== undefined) {
    const base =
      businessUpdate.subscriptionExpiresAt === undefined && business.subscriptionExpiresAt
        ? new Date(business.subscriptionExpiresAt)
        : null;
    const start = base && base.getTime() > Date.now() ? base : new Date();
    businessUpdate.subscriptionExpiresAt = new Date(start.getTime() + data.extendDays * 86400000);
    businessUpdate.neverExpires = false;
    businessUpdate.subscriptionStatus = "ACTIVE";
  }

  // Mirror top-level activation/deactivation between Restaurant and Business.
  if (data.isActive !== undefined) businessUpdate.isActive = data.isActive;

  const [updated] = await prisma.$transaction([
    prisma.restaurant.update({ where: { id: restaurantId }, data: restaurantUpdate }),
    prisma.business.update({ where: { id: business.id }, data: businessUpdate }),
  ]);
...
export async function DELETE(
  ...
  try {
    ...
    // and finally the Business tenant.
    await prisma.$transaction([
      ...
      ...(restaurant?.businessId ? [prisma.business.delete({ where: { id: restaurant.businessId } })] : []),
    ]);
