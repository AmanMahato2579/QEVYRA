...
export async function POST(req: Request) {
  ...
  const restaurantMetadata = await prisma.restaurant.findUnique({
  ...
  if (!restaurantMetadata?.business) return NextResponse.json({ error: "Restaurant is unavailable" }, { status: 404 });
  const access = await getEffectiveAccess(restaurantMetadata.business);
  if (!canBook(access, restaurantMetadata.bookingsEnabled)) {
